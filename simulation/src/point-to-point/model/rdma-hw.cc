...
TypeId RdmaHw::GetTypeId (void)
{
	static TypeId tid = TypeId ("ns3::RdmaHw")
		...
		.AddAttribute("GPUsNum",
				...
				MakeUintegerChecker<uint32_t>())
		.AddAttribute("InterDCbps",
				"the bandwidth between DCs, used for routing",
				DataRateValue(DataRate("400Gb/s")),
				MakeDataRateAccessor(&RdmaHw::m_inter_dc_bps),
				MakeDataRateChecker())
		.AddAttribute("IsHierarchical",
				"whether the simulation is hierarchical, used for equal bandwidth computation",
				BooleanValue(false),
				MakeBooleanAccessor(&RdmaHw::m_is_hierarchical),
				MakeBooleanChecker())
...
void RdmaHw::AddQueuePair(uint32_t src, uint32_t dest, uint64_t tag, uint64_t size, uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint32_t win, uint64_t baseRtt, bool isLossy, Callback<void> notifyAppFinish, Callback<void> notifyAppSent){
	...
	if (m_enable_equal_bw && (src/m_gpus_per_dc != dest/m_gpus_per_dc) && (src<m_gpus_num && dest<m_gpus_num)) {//different dcs and src and dst are both gpu (not nvswitch)
		// The number of chunks, hence flows, that the data (for DP) is split into, is given by the number of participants in the ring -1
		// In hierarchical simulation, since the number of participants in inter-dc rings is 2, only one flows per ring will be generated
		// The number of inter-dc rings for DP is equal to the number of GPUs in the leading replicas
		uint32_t dividing_factor = m_gpus_per_server;
		if (!m_is_hierarchical) {
		// In baseline simulation, the number of inter-dc rings for DP is still equal to the number of GPUs in each node
		// However, the size of each ring is equal to the number of nodes in the DCs, so the number of flows generated by each operation is equal to the number of nodes - 1
		dividing_factor *= (m_gpus_num/m_gpus_per_server - 1);
		}
		if (m_bps > m_inter_dc_bps/dividing_factor) {
			m_bps = m_inter_dc_bps/dividing_factor;
		}
		std::cout << "different dcs, adjust m_bps to: " << m_bps.GetBitRate() << " divided by dividing_factor: " << dividing_factor << std::endl;
	}
	qp->m_rate = m_bps;
	qp->m_max_rate = m_bps;
	if (m_cc_mode == 1){
