...
Ptr<RdmaQueuePair> RdmaHw::AddQueuePair(
  ...
  Ptr<Packet> mtuPacket = GenDataPacket(qp, m_mtu, 0);
...
Ptr<Packet> RdmaHw::GetNxtPacket(Ptr<RdmaQueuePair> qp) {
  ...
  Ptr<Packet> p = GenDataPacket(qp, payload_size, seq_to_send);
  // update state if it is not a retransmission
...
Ptr<Packet> RdmaHw::GenDataPacket(Ptr<RdmaQueuePair> qp, uint32_t pkt_size, uint64_t seqno) {
